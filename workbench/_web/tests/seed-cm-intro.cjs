 ...
 * UI E2E (B1/D1/F1) can exercise the heatmap, the last-row toggle, and the
 * history rail WITHOUT a model run / NDIF. Run under node before Playwright.
 *
...
const sourceData = buildLensData(" Paris");
...
function lensRunData(slot, finalToken) {
...
const insRun = db.prepare(
    "INSERT INTO lens_runs (id, workspace_id, chart_id, model, prompt, data, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
);
const versions = [
    ...
    { id: "33333333-3333-4333-8333-333333333333", prompt: TOKENS.join(""), tok: " Paris" },
];
versions.forEach((v, i) => {
    insRun.run(v.id, WS_ID, CHART_ID, MODEL, v.prompt, lensRunData("source", v.tok), now + i * 1000);
});

console.log(`Seeded workspace ${WS_ID}, chart ${CHART_ID} (cm-intro), ${versions.length} history rows -> ${DB_PATH}`);
db.close();
