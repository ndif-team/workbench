...
function mockBackendAPIs(page: Page) {
  ...
  page.route(`${BACKEND}/models/`, (route) =>
    route.fulfill({
      json: [
        { name: "gpt2", is_chat: false, n_layers: 12, params: "124M", gated: false, allowed: true },
      ],
    }),
  );
  ...
  page.route(`${BACKEND}/models/start-prediction`, (route) =>
    route.fulfill({
      json: {
        job_id: null,
        data: { idx: 4, ids: [262, 318, 373], probs: [0.08, 0.05, 0.04], texts: [" the", " is", " to"] },
      },
    }),
  );
  ...
  page.route(`${BACKEND}/lens/start-line`, (route) =>
    route.fulfill({
      json: {
        job_id: null,
        data: [
          { id: " the", data: Array.from({ length: 12 }, (_, i) => ({ x: i, y: Math.random() * 0.3 })) },
          { id: " is", data: Array.from({ length: 12 }, (_, i) => ({ x: i, y: Math.random() * 0.2 })) },
        ],
      },
    }),
  );
    ...
    route.fulfill({
      json: {
        ...
        data: {
          ...
          entropy: Array.from({ length: 12 }, () => Array.from({ length: 3 }, () => 2.5 + Math.random())),
        },
      },
    }),
  );
  ...
  page.route(`${BACKEND}/activation_patching/start`, (route) =>
    route.fulfill({
      json: {
        job_id: null,
        data: {
          lines: Array.from({ length: 3 }, () => Array.from({ length: 12 }, () => Math.random())),
          ranks: Array.from({ length: 3 }, () => Array.from({ length: 12 }, () => Math.floor(Math.random() * 100))),
          prob_diffs: Array.from({ length: 3 }, () => Array.from({ length: 12 }, () => (Math.random() - 0.5) * 0.2)),
...
export async function gotoFreshAPWorkspace(page: Page, opts?: {
  srcPrompt?: string;
  tgtPrompt?: string;
  srcPos?: number[];
  tgtPos?: number[];
