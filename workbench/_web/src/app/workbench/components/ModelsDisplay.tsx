...
function BaseModelCard({ model }: { model: { name: string; allowed: boolean; n_layers: number; params: string } }) {
  ...
  const darkGradient = 'linear-gradient(135deg, rgba(59, 130, 246, 0.35) 0%, rgba(96, 165, 250, 0.25) 50%, rgba(37, 99, 235, 0.15) 100%)'; // brighter blue shades
...
function ChatModelCard({ model }: { model: { name: string; allowed: boolean; n_layers: number; params: string } }) {
  const [isHovered, setIsHovered] = useState(false);
  const [isDark, setIsDark] = useState(false);
  ...
  const darkGradient = 'linear-gradient(135deg, rgba(147, 51, 234, 0.35) 0%, rgba(168, 85, 247, 0.25) 50%, rgba(126, 34, 206, 0.15) 100%)';
...
export function ModelsDisplay() {
  ...
  return (
    <div 
      ...
      style={{
        border: isDark ? '2px solid rgba(148, 163, 184, 0.3)' : '2px solid rgba(148, 163, 184, 0.25)',
    ...
    >
      <button
        ...
        style={{
          backgroundColor: isExpanded ? 'transparent' : (isDark ? 'rgba(51, 65, 85, 0.3)' : 'rgba(226, 232, 240, 0.5)')
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.backgroundColor = isDark ? 'rgba(51, 65, 85, 0.4)' : 'rgba(226, 232, 240, 0.7)';
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.backgroundColor = isExpanded ? 'transparent' : (isDark ? 'rgba(51, 65, 85, 0.3)' : 'rgba(226, 232, 240, 0.5)');
        }}
      >
                    ...
                    <Tooltip key={model.name}>
                      <TooltipTrigger asChild>
                        <div 
                          ...
                          style={{
                            borderColor: '#93c5fd',
                            backgroundImage: 'linear-gradient(to bottom right, #dbeafe, #eff6ff, #eff6ff)'
                          }}
                        >
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex-shrink-0 w-2 h-2 rounded-full mt-0.5" style={{ backgroundColor: '#60a5fa' }} />
                            </div>
                            <div className="flex items-center justify-between">
                              <div 
                                className="text-[10px] font-medium uppercase tracking-wide"
                    ...
                    <Tooltip key={model.name}>
                      <TooltipTrigger asChild>
                        <div 
                          ...
                          style={{
                            borderColor: '#d8b4fe',
                            backgroundImage: 'linear-gradient(to bottom right, #f3e8ff, #faf5ff, #faf5ff)'
                          }}
                        >
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex-shrink-0 w-2 h-2 rounded-full mt-0.5" style={{ backgroundColor: '#c084fc' }} />
                            </div>
                            <div className="flex items-center justify-between">
                              <div 
                                className="text-[10px] font-medium uppercase tracking-wide"
